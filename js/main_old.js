...
function expandShown(board, elCell, cellI, cellJ){
  ...
  //              console.log("show : ", i, j);
                board[i][j].show = true;
            }
        }
    }

    // loop over 2-cell distance
   for ( var i =cellI-2; i <= cellI+2; i++ ) {
        for (var j =cellJ-2; j <= cellJ+2; j++) {        
            ...
            //  console.log("expandShown: ", i, j);
            if (    (board[i][j].value !== MINE_SYMBOL) && 
                    !hasVisibleNumberNbr(board, cellI,cellJ)) {
  //              console.log("show : ", i, j);
                board[i][j].show = true;
            }
        }
...
// is cell(i,j) has visible number neighbor??
function hasVisibleNumberNbr(board, cellI,cellJ){
    var res = false;
   for (var i =cellI-1; i <= cellI+1; i++) {
        for (var j =cellJ-1; j <= cellJ+1; j++) {        
            if ( i === cellI && j === cellJ ) continue;
            ...
            if ( j < 0 || j > board[0].length-1) continue;
            var cell = board[i][j] ;
            if ((cell.value > 0) && (cell.value <= 9) && (cell.show)) {
                res = true;
                break;
            }
        }
